...
public class PhotoProcessor {
  ...
  private Photo resize(Photo photo, int scaledWidth, int scaledHeight) throws IOException {
    ...
    try (InputStream inputStream = new ByteArrayInputStream(photo.getPhotoData())) {
      ...
      if (inputImage != null) {
        ...
        BufferedImage outputImage =
            new BufferedImage(scaledDimension.width, scaledDimension.height, inputImage.getType());
        ...
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
          ...
          return new Photo(
              photo.getDownloadedDate(), photo.getExtension(), outputStream.toByteArray());
        }
      }
    } catch (IOException e) {
  ...
  private Dimension rescaleKeepingRatio(
      BufferedImage inputImage, int preferredWidth, int preferredHeight) {
