...
public class SchoolDemo {
  ...
  public void showClass() {
    SchoolClass schoolClass = school.findClass("1A", "humane").iterator().next();
    ...
    for (Subject subject : schoolClass.getSubjects()) {
      ...
      System.out.format(
          "Subject teacher: %s %s\n",
          subject.getTeacher().getName(), subject.getTeacher().getSurname());
    }
    System.out.println();
  }
  ...
  public void showScheduleForTeacher() {
    Teacher teacher = (Teacher) school.findPerson("Han", "Solo").iterator().next();
  ...
  private void renderSchedule(Collection<Term> terms, String description) {
    ...
    for (Term term : terms) {
      ...
      System.out.format("\tStart time: %s \n", timeFormat.format(term.getStartTime()));
