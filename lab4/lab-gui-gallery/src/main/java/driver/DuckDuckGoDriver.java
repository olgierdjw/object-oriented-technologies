...
public class DuckDuckGoDriver {
  ...
  private static final Pattern VQD_ID =
      Pattern.compile("vqd=([\\d-]+)&", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
  ...
  private static URI getScraperUri(String queryRequest) {
    var scraperRequest =
        "http://api.scraperapi.com?api_key=%s&url=%s".formatted(Main.SCRAPER_API_KEY, queryRequest);
    return URI.create(scraperRequest);
  }

  private static HttpResponse<String> sendRequest(HttpClient client, URI uri)
      throws IOException, InterruptedException {
    var request = HttpRequest.newBuilder().uri(uri).header("User-Agent", USER_AGENT).GET().build();
    return client.send(request, HttpResponse.BodyHandlers.ofString());
  }

  private static List<String> extractPhotoUrls(String html, String searchQuery)
      throws IOException, InterruptedException {
    ...
    if (matcher.find()) {
      ...
      var queryRequest =
          String.format("https://duckduckgo.com/i.js?q=%s&o=json&vqd=%s", searchQuery, vqdId);
      URI uri = getScraperUri(queryRequest);
      var client = HttpClient.newHttpClient();
      HttpResponse<String> response = sendRequest(client, uri);
